...
icc.no.nine.genes <- icc.all[!icc.all$uniprotID %in% gsub('.clean.itan.split|.itan.split', '', nine.genes),]
...
for (seed in 0:4) {
  ...
  # write
  dir.create(paste0('ICC.seed.', seed, '/ALL.itan/'), showWarnings = F)
  # drop score.labels column
  icc.train$score.label <- NULL
  icc.test$score.label <- NULL
  write.csv(icc.train, paste0('ICC.seed.', seed, '/ALL.itan/training.csv'))
  write.csv(icc.test, paste0('ICC.seed.', seed, '/ALL.itan/testing.csv'))
}
# do itan and no itan
icc.noitan <- icc.all[!grepl('Itan', icc.all$data_source),]
icc.itan <- icc.all[grepl('Itan', icc.all$data_source),]
dir.create(paste0('ICC.seed.', 0, '/ALL.itan.only/'))
write.csv(icc.noitan, paste0('ICC.seed.', 0, '/ALL.itan.only/testing.csv'))
write.csv(icc.itan, paste0('ICC.seed.', 0, '/ALL.itan.only/training.csv'))
