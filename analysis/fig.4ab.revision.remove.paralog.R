...
for (i in 1:length(genes)) {
  for (fold in 0:4) {
    ...
    test.noparalog.result <- read.csv(paste0('PreMode.revision.remove.all.paralog/', genes[i], '/',
    ...
    PreMode.noparalog.auc <- plot.R2(test.noparalog.result[,names(task.dic[[genes[i]]])], test.noparalog.result[,paste0("logits.", 0:(task.length-1))], bin = grepl('bin', genes[i]))
...
ggsave(paste0('figs/fig.4a.revision.remove.paralog.pdf'), p, height = 8, width = 4)
...
p <- ggplot(uniq.model.result.plot.plot, aes(x=stab.rho, y=func.rho, col=model)) +
  ...
  ggeasy::easy_center_title()
ggsave('figs/fig.4b.revision.remove.paralog.pdf', p, height=4, width=5)
