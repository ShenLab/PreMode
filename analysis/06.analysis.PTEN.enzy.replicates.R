...
# ground.truth <- read.csv(paste0('/share/pascal/Users/gz2294/Data/DMS/MAVEDB/PTEN/assay.2.csv'), row.names = 1)
ground.truth <- read.csv(paste0('/share/pascal/Users/gz2294/Data/DMS/MAVEDB/raw/urn_mavedb_00000054-a-1_scores.csv'), skip = 4)
ground.truth$score <- rowMeans(ground.truth[,paste0(c("A1", "A2", "A3", "B1", "B2", "B3"), "_score")], na.rm = T)
# Next set up metrics
all.premode <- list(c(), c(), c(), c(), c(), c())
all.baseline <- list(c(), c(), c(), c(), c(), c())
all.replicates <- list()
for (r in 1:6) {
  replicate <- read.csv(paste0('PreMode.inference/PTEN.enzy/replicate.', r, '.csv'))
  training <- read.csv(paste0('/share/pascal/Users/gz2294/Data/DMS/MAVEDB/PTEN.replicate.enzy.rest.', 
  ...
  for (i in 1:dim(replicate.unique)[1]) {
    ...
    replicate.unique$ground.truth[i] <- ground.truth$score[ground.truth$hgvs_pro==replicate$aaChg[i]]
...
save(all.premode, all.baseline, all.replicates, file = 'figs/05.PTEN.enzy.replicates.RData')
npoints <- table(all.replicates[[1]]$observations)
npoints <- c(sum(npoints), npoints)
names(npoints)[1] <- "all"
to.plot <- data.frame(RMSE=sqrt(c(unlist(all.premode), 
                            unlist(all.baseline))), 
                      replicate=paste0(rep(rep(names(npoints), each=6), 2), " : ",
                                       rep(rep(npoints, each=6), 2)),
                      model=c(rep("PreMode", length(npoints)*6), rep("Experiment", length(npoints)*6)))
write.csv(to.plot, file = 'figs/05.PTEN.enzy.replicates.csv')
library(ggplot2)
...
# ggsave("figs/05.PTEN.enzy.replicates.pdf", height = 5, width = 6)
...
# ggsave(filename = 'figs/05.PTEN.enzy.replicates.PreMode.pdf', p, width = 20, height = 5)
...
library(patchwork)
p <- (p1 + p2) / p3
ggsave(filename = 'figs/05.PTEN.enzy.replicates.PreMode.pdf', p, width = 20, height = 10)
