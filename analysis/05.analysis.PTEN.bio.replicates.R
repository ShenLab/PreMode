...
for (r in 1:8) {
  ...
  for (i in 1:dim(replicate.unique)[1]) {
    ...
    replicate.unique$base.line.2[i] <- mean(c(replicate.baseline$score[i]))
    replicate.unique$ground.truth[i] <- ground.truth$score[ground.truth$VarID==replicate$aaChg[i]]
...
p1 <- ggplot(to.plot, aes(x=replicate, y=RMSE, fill=model, col=model)) + 
  ...
  geom_segment(data = to.connect,
               aes(x    = as.numeric(as.factor(replicate)) + 0.12, 
                   xend = as.numeric(as.factor(replicate)) - 0.12,
...
for (r in 1:length(all.replicates)) {
  ...
  diff.plots[[r]] <- ggplot(all.replicates[[r]], aes(x=Experiment.PreMode.diff, y=Experiment.Groundtruth.diff)) +
...
for (i in 1:dim(replicate.position.unique)[1]) {
  replicate.position.unique$PreMode.Experiment.RMSE.diff[i] <- mean(replicate.unique$PreMode.Experiment.RMSE.diff[replicate.unique$pos.orig==replicate.position.unique$pos.orig[i]], na.rm = T)
...
p6 <- protein.visualize('P60484', 'PTEN : # replicates', to.plot, 'to.plot',
                        fill.colors = c("red", "white", "blue"),
                        fill.values = c(0, 0.5, 1))
...
ggsave(filename = 'figs/05.PTEN.bio.replicates.PreMode.2.pdf', p4, width = 20, height = 10)
