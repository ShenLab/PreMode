...
for (i in 1:length(genes)) {
  ...
  if (!file.exists(paste0('PreMode.inference/', gene, '.all.mutants.RDS'))) {
    ...
    gene.test <- read.csv(paste0('/share/vault/Users/gz2294/Data/DMS/Itan.CKB.Cancer.good.batch/pfams.0.8.seed.', fold, '/',
                                 gene, '/testing.csv'))
    gene.all.emb.orig <- read.csv(paste0('PreMode.inference/', gene, '/all.mutants.pretrain.csv'))
    ...
    gene.all.emb.orig$aaChg <- paste0('p.', gene.all.emb.orig$ref, gene.all.emb.orig$pos.orig, gene.all.emb.orig$alt)
    ...
    gene.all.emb <- gene.all.emb.orig[,colnames(gene.all.emb.orig)[startsWith(colnames(gene.all.emb.orig), 'X')]]
    gene.all.emb$X <- NULL
    gene.all.emb$X.1 <- NULL
    ...
    labels.all <- data.frame(pos=gene.all.emb.orig$pos.orig,
                             aaChg=gene.all.emb.orig$aaChg,
                             split=labels$split[match(gene.all.emb.orig$aaChg, labels$aaChg)],
                             label=labels$label[match(gene.all.emb.orig$aaChg, labels$aaChg)])
    labels <- labels.all
    ...
    emb.clust <- cluster.leiden(gene.all.emb, pca_only = T)
    emb.clust$split <- labels.all$split
    emb.clust$label <- 'GoF'
    emb.clust$label[labels.all$label==-1] <- 'LoF'
    emb.clust$pos <- labels.all$pos
    ...
    saveRDS(emb.clust, file = paste0('PreMode.inference/', gene, '.all.mutants.RDS'))
  } else {
    emb.clust <- readRDS(paste0('PreMode.inference/', gene, '.all.mutants.RDS'))
  }
  library(ggplot2)
  library(patchwork)
  ...
  p2 <- ggplot(emb.clust, aes(x=PC1, y=PC2, col=domain)) +
    geom_point(alpha=0.2, size=0.5) + theme_bw() + 
    ...
    ggeasy::easy_center_title()
  p3 <- ggplot(emb.clust, aes(x=PC1, y=PC2, col=connectivity/max(connectivity), shape=split)) +
    geom_point() + theme_bw() + ggtitle(paste0(gene.names[i])) + 
    scale_shape_manual(values=c(1, 15, 12))+
    scale_color_gradientn(colours = c('yellow', 'purple', 'black')) +
    labs(col='connectivity') +
    ggeasy::easy_center_title()
...
ggsave(filename = 'figs/10.01.analysis.all.mutants.zero.shot.pdf', p, height = 30, width = 12)
...
ggsave(filename = 'figs/10.03.analysis.all.mutants.zero.shot.pdf', p, height = 30, width = 12)
