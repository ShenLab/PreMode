...
result.plot <- result.plot[result.plot$model %in% c("PreMode/",
                                                    # "PreMode.ALL.itan.only/",
                                                    "PreMode.all/",
                                                    "random.forest"
),]
model.dic <- c("PreMode/"="1: PreMode",
               "PreMode.noPretrain/"="3: PreMode: no pretrain",
               "PreMode.all/"="4: PreMode: all genes",
               "PreMode.ALL.nogeneoverlap/"="5: PreMode: all genes\nremove gene overlap",
               "PreMode.ALL.itan.only/"="5: PreMode: all genes\nLoGoFunc data only",
...
ggsave(paste0('figs/fig.5a.revision.nogeneoverlap.pdf'), p1, height = 5.1, width = 6)

saveRDS(result.plot, 'figs/fig.5a.revision.nogeneoverlap.plot.RDS')
