...
wil.stat <- wilcox.test(ALL$FoldXddG[!is.na(ALL$plot.label) & ALL$plot.label=='Good Genes' & ALL$score==-1],
                        ALL$FoldXddG[!is.na(ALL$plot.label) & ALL$plot.label=='Good Genes' & ALL$score==1])
p1 <- ggplot(ALL[!is.na(ALL$plot.label) & ALL$plot.label=='Good Genes',],
             aes(x=FoldXddG, col=score.label)) + geom_density() + 
  ggpp::geom_text_npc(data=data.frame(x="right", y="middle",
            ...
            col='black') + 
  theme_bw() + 
  scale_x_continuous(trans = ggallin::pseudolog10_trans, limits = c(-5, 30)) +
  # xlim(0, 100) +
  ggtitle('Stab decrease prediction: PTM distance') + ggeasy::easy_center_title()
wil.stat <- wilcox.test(ALL$FoldXddG[!is.na(ALL$plot.label) & ALL$plot.label=='Bad Genes' & ALL$score==-1],
                        ALL$FoldXddG[!is.na(ALL$plot.label) & ALL$plot.label=='Bad Genes' & ALL$score==1])
p5 <- ggplot(ALL[!is.na(ALL$plot.label) & ALL$plot.label=='Bad Genes',],
  ...
  scale_x_continuous(trans = ggallin::pseudolog10_trans, limits = c(-5, 30)) +
  # xlim(0, 100) +
  ggtitle('Stab increase prediction: PTM distance') + ggeasy::easy_center_title()

library(patchwork)
p <- (p1 + p5) + plot_layout(ncol = 2)
ggsave(filename = 'figs/02.02.explain.stab.pdf', p, width = 12, height = 2)
