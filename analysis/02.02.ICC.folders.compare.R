...
# result <- result[result$task.type %in% c("Pfam", "Gene", "Gene.Domain"),]
# result <- result[result$task.type %in% c("Gene"),]
...
for (i in 1:dim(result)[1]) {
  ...
  baseline.auc.7 <- plot.AUC(test.result$score[baseline.result$data_source != "Itan"],
                             (test.result$itan.gof/test.result$itan.lof)[baseline.result$data_source != "Itan"])
  to.append <- result[rep(i, length(base.dirs)+6), ]
  to.append$auc <- c(baseline.PreModes, baseline.auc.noItan$auc, baseline.auc.large.window.noItan$auc, 
                     baseline.auc.4$auc, baseline.auc.5$auc,
                     baseline.auc.6$auc, baseline.auc.7$auc)
...
saveRDS(result.gof, 'figs/02.02.ICC.folders.compare.RDS')
