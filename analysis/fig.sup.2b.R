...
for (i in 1:length(auc.list)) {
  ...
  if (model.names[i] == "PreMode.noparalog") {
    model.auc$model <- paste0("PreMode", "(", round(auc.list[[i]]$auc, 3), ")\n(remove paralog in training)")
    model.name <- c(model.name, paste0("PreMode", "(", round(auc.list[[i]]$auc, 3), ")\n(remove paralog in training)"))
  } else {
