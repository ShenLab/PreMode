...
for (i in 1:length(genes)) {
  ...
  gene.train.emb <- gene.train.emb.orig[,colnames(gene.train.emb.orig)[startsWith(colnames(gene.train.emb.orig), 'jacob.')]]
  gene.train.emb$X <- NULL
  gene.train.emb$X.1 <- NULL
  gene.test.emb <- gene.test.emb.orig[,colnames(gene.test.emb.orig)[startsWith(colnames(gene.test.emb.orig), 'jacob.')]]
  ...
  source('/share/vault/Users/gz2294/Pipeline/plot.genes.scores.heatmap.R')
  # try to plot complex heatmap
  cmplx.heatmap <- rbind(gene.train.emb, gene.test.emb, gene.val.emb)
  rownames(cmplx.heatmap) <- labels$aaChg
  cmplx.heatmap.labels <- labels[,c("split", "label")]
  cmplx.heatmap.labels.num <- data.frame(pos=labels$pos)
  plot.complex.heatmap(cmplx.heatmap, row.anno = cmplx.heatmap.labels, numeric.row.anno = cmplx.heatmap.labels.num)
  # try pca
  emb.clust <- cluster.leiden(rbind(gene.train.emb, gene.test.emb, gene.val.emb), scale = T)
  emb.clust$split <- labels$split
  emb.clust$aaChg <- labels$aaChg
  emb.clust$label <- 'GoF'
  ...
  library(ggplot2)
  library(patchwork)
  p1 <- ggplot(emb.clust, aes(x=PC1, y=PC2, col=label, shape=split, label=aaChg)) +
