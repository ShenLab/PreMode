...
for (o in 1:length(genes)) {
  ...
  gene.af2.file <- paste0("~/Data/af2_uniprot/alphafold2_v4/", substr(gene, 1, 3), "/AF-",
                          gene, '-F', 1,
                          '-model_v4.pdb.gz')
  ...
  if (!"logits" %in% colnames(gene.result)) {
    ...
    fill.name <- c("Benign-Patho", "GoF", "LoF", "LoF-GoF")
    for (j in 1:4) {
      if (j == 1) {
        all.training.to.plot.plot <- all.pretrain
        all.training.to.plot.plot$score <- as.character(all.training.to.plot.plot$score)
      } else {
        all.training.to.plot.plot <- all.training.to.plot
        all.training.to.plot.plot$score <- as.character(all.training.to.plot.plot$score)
      }
      ps[[j]] <- ggplot() +
        geom_tile(data=gene.result, aes_string(y="pos.orig", x="alt", fill=col.to.plot[j])) + labs(fill=col.to.plot[j]) +
        scale_fill_gradientn(colors = c("light blue", "white", "pink"), na.value = 'grey') +
        scale_y_continuous(trans = "reverse", breaks=seq(0, nchar(gene.seq), 50), minor_breaks = seq(0, nchar(gene.seq), 10)) + 
        labs(fill=paste0("Pred: ", fill.name[j])) +
        ggnewscale::new_scale_fill() +
        geom_tile(data=all.training.to.plot.plot, aes(y=pos.orig, x=alt, fill=score, width=1, height=1)) +
        scale_fill_manual(values = c("blue", "white", "red"), breaks=c("-1", "0", "1")) +
        labs(fill=paste0("Label: ", fill.name[j])) +
        ggnewscale::new_scale_fill() +
        geom_tile(data=secondary.df.to.plot, aes(y=pos.orig, x=alt, fill=ANNO_secondary, width=1, height=1)) +
        ggnewscale::new_scale_fill() +
        geom_tile(data=rsa.df, aes(y=pos.orig, x=alt, fill=ANNO_RSA, width=1, height=1)) +
        scale_fill_gradientn(colors = c("grey", "blue")) +
        ggnewscale::new_scale_fill() +
        geom_tile(data=plddt.df, aes(y=pos.orig, x=alt, fill=ANNO_pLDDT, width=1, height=1)) +
        scale_fill_gradientn(colors = c("orange", "yellow", "lightblue", "blue")) +
        ggnewscale::new_scale_fill() +
        geom_tile(data=unique.df.to.plot, aes(y=pos.orig, x=alt, fill=ANNO_domain_type, width=1, height=1),show.legend = F) +
        ggnewscale::new_scale_fill() +
        geom_tile(data=multiple.df.to.plot, aes(y=pos.orig, x=alt, fill=ANNO_domain_type, width=1, height=1),show.legend = F)
      ps[[j]] <- ps[[j]] +
