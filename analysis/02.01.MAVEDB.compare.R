...
for (i in 1:length(genes)) {
  ...
  if (!'RosettaddG' %in% colnames(assay)) {
    assay <- rbind(read.csv(paste0('/share/vault/Users/gz2294/Data/DMS/MAVEDB/', gene, '/training.csv'), row.names = 1),
                   read.csv(paste0('/share/vault/Users/gz2294/Data/DMS/MAVEDB/', gene, '/testing.csv'), row.names = 1))
    assay <- uniprot.table.add.annotation.parallel(assay, 'ESM1b')
    assay <- uniprot.table.add.annotation.parallel(assay, 'AlphaMissense')
    assay <- uniprot.table.add.annotation.parallel(assay, 'dbnsfp')
    assay <- uniprot.table.add.annotation.parallel(assay, 'EVE')
    assay <- uniprot.table.add.annotation.parallel(assay, 'gMVP')
    assay <- uniprot.table.add.annotation.parallel(assay, 'FoldXddG')
    assay <- uniprot.table.add.annotation.parallel(assay, 'dssp')
    assay <- uniprot.table.add.annotation.parallel(assay, 'pLDDT')
    assay <- uniprot.table.add.annotation.parallel(assay, 'conservation')
    assay <- uniprot.table.add.annotation.parallel(assay, 'RosettaddG')
    write.csv(assay, file = paste0('/share/vault/Users/gz2294/Data/DMS/MAVEDB/', gene, '/ALL.annotated.csv'))
  }
  print(gene)
}
...
ggsave('figs/02.01.MAVEDB.assays.compare.pdf', p, height = 10, width = 10)
