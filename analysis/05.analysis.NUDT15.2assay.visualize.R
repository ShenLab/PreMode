...
for (gene in genes) {
  ...
  for (fold in folds) {
    if (fold == -1) {
      ...
      training.file <- read.csv(paste0('~/Data/DMS/ClinVar.HGMD.PrimateAI.syn/all.clinvar.4.splits/training.csv'))[,c("HGNC", "uniprotID", "pos.orig", "ref", "alt", "score", "data_source")]
      ...
      training.file <- read.csv(paste0('~/Data/DMS/MAVEDB/NUDT15/train.seed.', fold, '.csv'))[,c("pos.orig", "ref", "alt", "score.1", "score.2")]
      training.file$score <- NA
      testing.file <- read.csv(paste0('~/Data/DMS/MAVEDB/NUDT15/test.seed.', fold, '.csv'))[,c("pos.orig", "ref", "alt", "score.1", "score.2")]
    ...
    if (!"logits" %in% colnames(gene.result) | fold != -1) {
      ...
      for (j in 1:3) {
        ps[[j]] <- ggplot() +
          ...
          theme_bw() + 
          ggtitle("NUDT15") + ggeasy::easy_center_title()
      }
