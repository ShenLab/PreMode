...
# remove itan tasks
result.plot <- result.plot[grepl('itan', result.plot$task.id),]
pick.cond <- 'auc'
# get unique models
...
result.plot$model[grepl('.split.by.pos.itan', result.plot$task.id)] <- paste0(result.plot$model[grepl('.split.by.pos.itan', result.plot$task.id)], '.by.pos')
result.plot$task.id <- gsub('.split.by.pos.itan', '', result.plot$task.id)
result.plot$task.id <- gsub('.itan.split', '', result.plot$task.id)

result.plot$task.name[result.plot$task.id == "Q14524.clean"] <- "Gene: SCN5A"
result.plot <- result.plot[result.plot$model %in% c("PreMode/",
                                                    "PreMode/.by.pos",
                                                    "PreMode.revision.remove.glof.paralog/.by.pos",
                                                    # "PreMode.ALL.nogeneoverlap/",
                                                    # "PreMode.ALL.nogeneoverlap/.by.pos",
                                                    "Itan.1",
                                                    "Itan.1.by.pos"
                                                    # "ESM.SLP/",
                                                    # "ESM.SLP/.by.pos",
                                                    # "random.forest", "random.forest.by.pos"
                                                    ),]
model.dic <- c("PreMode/"="PreMode",
               "PreMode/.by.pos"="PreMode: split by pos",
               "PreMode.ALL.nogeneoverlap/"="PreMode: all genes\nno gene overlap",
               "PreMode.ALL.nogeneoverlap/.by.pos"="PreMode: all genes\nno gene overlap\nsplit by pos",
               "PreMode.revision.remove.glof.paralog/"="PreMode: remove paralog",
               "PreMode.revision.remove.glof.paralog/.by.pos"="PreMode: remove paralog\nsplit by pos",
               "random.forest"="Random Forest",
               "random.forest.by.pos"="Random Forest: split by pos",
               "ESM.SLP/"="ESM + SLP",
               "ESM.SLP/.by.pos"="ESM + SLP: split by pos",
               "Itan.1"="LoGoFunc",
               "Itan.1.by.pos"="LoGoFunc: split by pos")
...
uniq.model.result.plot$model.type[uniq.model.result.plot$model %in% c('ESM + SLP', "ESM + SLP: split by pos", 
                                                                      'Random Forest', "Random Forest: split by pos", 
                                                                      "LoGoFunc", "LoGoFunc: split by pos")] <- 'Baselines'
uniq.model.result.plot$model.type[uniq.model.result.plot$model %in% c("PreMode", "PreMode: split by pos",
                                                                      "PreMode: all genes\nno gene overlap", "PreMode: all genes\nno gene overlap\nsplit by pos",
                                                                      "PreMode: remove paralog", "PreMode: remove paralog\nsplit by pos")] <- 'PreMode'
uniq.model.result.plot$model.type <- factor(uniq.model.result.plot$model.type, 
                                            levels = c('PreMode', 'Baselines'))
uniq.model.result.plot$model <- factor(uniq.model.result.plot$model,
                                       levels = c('PreMode', 'PreMode: split by pos', 
                                                  ...
                                                  "LoGoFunc", "LoGoFunc: split by pos"))
...
p <- ggplot(uniq.model.result.plot, aes(x=model, y=auc, col=model.type)) +
  ...
  theme_bw() + ylim(0.4, 0.9) + ggtitle('PreMode compare to LoGoFunc') +
  ...
  ggeasy::easy_center_title()
ggsave('figs/fig.5d.by.itan.pos.pdf', p, height=5, width=6)
