...
result.plot$model[grepl('.split.by.pos', result.plot$task.id)] <- paste0(
  result.plot$model[grepl('.split.by.pos', result.plot$task.id)], '.by.pos')
result.plot$task.id <- gsub('.split.by.pos', '', result.plot$task.id)
...
result.plot <- result.plot[result.plot$model %in% c(#"PreMode/",
                                                    ...
                                                    "PreMode.revision.remove.all.paralog/.by.pos",
...
model.dic <- c("PreMode/"="PreMode",
               ...
               "PreMode.revision.remove.all.paralog/"="PreMode: remove paralog",
               "PreMode.revision.remove.all.paralog/.by.pos"="PreMode: remove paralog\nsplit by pos",
               "random.forest"="Random Forest",
               "random.forest.by.pos"="Random Forest:\nsplit by pos",
               "ESM.SLP/"="ESM + SLP",
...
p1 <- ggplot(result.plot, aes(y=auc, x=task.name, col=model)) +
  ...
  ylim(0, 1) + xlab('task: Genetic Level Mode of Action') 
ggsave(paste0('figs/fig.5a.revision.remove.glof.paralog.pdf'), p1, height = 5.1, width = 6)
