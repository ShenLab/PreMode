...
ground.truth <- read.csv(paste0('/share/pascal/Users/gz2294/Data/DMS/MAVEDB/TPMT/assay.csv'), row.names = 1)
# Next set up metrics
all.premode <- list(c(), c(), c(), c(), c(), c(), c(), c())
all.baseline <- list(c(), c(), c(), c(), c(), c(), c(), c())
all.replicates <- list()
for (r in 1:8) {
  replicate <- read.csv(paste0('PreMode.inference/TPMT/replicate.', r, '.csv'))
  training <- read.csv(paste0('/share/pascal/Users/gz2294/Data/DMS/MAVEDB/TPMT.replicate.rest.', 
  ...
  for (i in 1:dim(replicate.unique)[1]) {
    ...
    replicate.baseline <- replicate[replicate$aaChg == replicate.unique$aaChg[i],]
    replicate.unique$base.line.1[i] <- mean(c(baseline$score), na.rm=T)
    replicate.unique$base.line.2[i] <- mean(c(replicate.baseline$score), na.rm=T)
  ...
  for (i in min(replicate.unique$other.observations, na.rm = T):max(replicate.unique$other.observations, na.rm = T)) {
    premode <- mean((replicate.unique$base.line.2[replicate.unique$other.observations==i] -
    ...
    baseline <- mean((replicate.unique$base.line.1[replicate.unique$other.observations==i] - 
                        replicate.unique$base.line.2[replicate.unique$other.observations==i])^2, na.rm = T)
    all.premode[[i]] <- c(all.premode[[i]], premode)
    all.baseline[[i]] <- c(all.baseline[[i]], baseline)
  }
  print(paste0('replicate ', r, ', PreMode: ', all.premode[[1]], ', Baseline: ', all.baseline[[1]]))
}
save(all.premode, all.baseline, all.replicates, file = 'figs/05.TPMT.bio.replicates.RData')
...
write.csv(to.plot, file = 'figs/05.TPMT.bio.replicates.csv')
library(ggplot2)
library(tidyverse)
...
# ggsave("figs/05.TPMT.bio.replicates.pdf", height = 5, width = 6)
...
ggsave('figs/05.TPMT.bio.replicates.PreMode.4.pdf', p7, height = 10, width = 20)
...
p3 <- protein.visualize('P60484', 'TPMT', replicate.unique, 'PreMode.Experiment.RMSE.diff',
...
p6 <- protein.visualize('P60484', 'TPMT : # replicates', to.plot, 'observations',
                        fill.colors = c("red", "white", "blue"),
                        fill.values = c(2, 5, 8))
to.plot <- replicate.position.unique
to.plot$alt <- "Replicate #"
...
# ggsave('figs/05.TPMT.bio.replicates.PreMode.5.pdf', plot = p, height = 15, width = 20)
# p7 <- protein.visualize('P60484', 'TPMT : # replicates', replicate.position.unique, 'observations',
#                         fill.colors = c("red", "white", "blue"),
...
# ggsave(filename = 'figs/05.TPMT.bio.replicates.PreMode.pdf', p, width = 20, height = 5)
# p2 <- protein.visualize('P60484', 'TPMT : Experiment RMSE', replicate.unique, 'Experiment.RMSE',
#                         fill.colors = c("purple", "yellow", "black"),
#                         fill.values = c(0, 0.75, 1.5))
# p3 <- protein.visualize('P60484', 'TPMT : PreMode', replicate.unique, 'PreMode.RMSE',
#                         fill.colors = c("purple", "yellow", "black"),
#                         fill.values = c(0, 0.75, 1.5))
library(patchwork)
p <- (p1 + p2) / p10 + plot_layout(heights = unit(c(5,12.5), units = 'inch'))
ggsave(filename = 'figs/05.TPMT.bio.replicates.PreMode.1.pdf', p, width = 20, height = 20)
ggsave(filename = 'figs/05.TPMT.bio.replicates.PreMode.2.pdf', p4, width = 20, height = 7.5)
