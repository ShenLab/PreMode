...
for (i in 1:dim(result)[1]) {
  ...
  if (!startsWith(result$task.name[i], "PF")) {
    ...
    tmp <- strsplit(result$task.name[i], ".chps")[[1]]
    if (grepl('.PF', result$task.name[i])) {
...
result.plot <- result.plot[result.plot$model %in% c("PreMode", "Baseline (No structure)", "Baseline (No ESM)"),]
...
ggsave(paste0('figs/02.02.ICC.PreMode.compare.pdf'), p, height = 6, width = 15)
