...
for (i in 1:length(genes)) {
  ...
  for (fold in 0:4) {
    ...
    auc <- plot.R2(gene.test.res[,c("score.1", "score.2")], gene.test.res[,c("logits.0", "logits.1")], bin = T)
    aucs <- c(aucs, auc$R2)
  }
  gene.train <- read.csv(paste0('/share/vault/Users/gz2294/Data/DMS/MAVEDB/PTEN.bin/train.seed.', fold, '.csv'))
  gene.test <- read.csv(paste0('/share/vault/Users/gz2294/Data/DMS/MAVEDB/PTEN.bin/test.seed.', fold, '.csv'))
  ...
  labels <- data.frame(split=c(rep("train", dim(gene.train.emb)[1]),
                       ...
                       alt=c(gene.train$alt[train.val.split['idx_train']+1],
                             gene.test$alt,
                             gene.train$alt[train.val.split['idx_val']+1]),
                       pos.orig=c(gene.train$pos.orig[train.val.split['idx_train']+1],
                             gene.test$pos.orig,
                             gene.train$pos.orig[train.val.split['idx_val']+1]),
                       logit=c(gene.train.emb.orig$logits[train.val.split['idx_train']+1],
                               gene.test.emb.orig$logits,
                               gene.train.emb.orig$logits[train.val.split['idx_val']+1]),
                       label=c(paste0(gene.train$score.1[train.val.split['idx_train']+1], ":",
                                      gene.train$score.2[train.val.split['idx_train']+1]),
                               paste0(gene.test$score.1, ":", gene.test$score.2),
                               paste0(gene.train$score.1[train.val.split['idx_val']+1], ":",
                                      gene.train$score.2[train.val.split['idx_val']+1])))
  ...
  emb.clust$label <- 'Stability: + / Enzyme: +'
  emb.clust$label[labels$label=='0:1'] <- 'Stability: + / Enzyme: -'
  emb.clust$label[labels$label=='1:0'] <- 'Stability: - / Enzyme: +'
  emb.clust$label[labels$label=='1:1'] <- 'Stability: - / Enzyme: -'
  emb.clust$pos <- labels$pos.orig
  ...
  emb.clust$logits <- labels$logit
  # get domain and zn fing information
  prot_data <- drawProteins::get_features("P60484")
  prot_data <- drawProteins::feature_to_dataframe(prot_data)
  prot_data$description[grepl('Phospho', prot_data$description) & prot_data$type=='MOD_RES'] <- 'Phospho'
  prot_data <- prot_data[!prot_data$type %in% c('VARIANT', 'VAR_SEQ', 'MUTAGEN', 'HELIX', 'STRAND', 'TURN', 'COMPBIAS', 'CHAIN', 'INIT_MET'),]
  emb.clust$domain <- NA
  ...
  p1 <- ggplot(emb.clust, aes(x=PC1, y=PC2, col=label, shape=split)) +
    geom_point(size=1, alpha=0.5) + theme_bw() + ggtitle(paste0(gene.names[i])) + 
  ...
  p3 <- ggplot(emb.clust, aes(x=PC1, y=PC2, col=connectivity/10, shape=split)) +
    geom_point(size=1, alpha=0.5) + theme_bw() + ggtitle(paste0(gene.names[i])) + 
    scale_shape_manual(values=c(1, 15, 12))+
    scale_color_gradientn(colours = c('yellow', 'purple', 'black')) +
    ggeasy::easy_center_title()
  plots[[i]] <- p1 + p2 + plot_layout(ncol = 2)
  plots.conn[[i]] <- p3 + p1 + plot_layout(ncol = 2)
}
p <- plots[[1]]
ggsave(filename = 'figs/10.04.analysis.PTEN.zero.shot.pdf', p, height = 5, width = 16)
p <- plots.conn[[1]]
ggsave(filename = 'figs/10.05.analysis.PTEN.zero.shot.pdf', p, height = 5, width = 14)
