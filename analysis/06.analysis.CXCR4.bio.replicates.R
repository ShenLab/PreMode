...
# ground.truth <- read.csv(paste0('/share/pascal/Users/gz2294/Data/DMS/MAVEDB/CXCR4/assay.2.csv'), row.names = 1)
ground.truth <- read.csv(paste0('/share/pascal/Users/gz2294/Data/DMS/MAVEDB/raw/urn_mavedb_00000048-a-1_scores.csv'), skip = 4)
...
all.replicates <- list()
for (r in 1:4) {
  replicate <- read.csv(paste0('PreMode.inference/CXCR4/replicate.', r, '.csv'))
  training <- read.csv(paste0('/share/pascal/Users/gz2294/Data/DMS/MAVEDB/CXCR4.replicate.rest.', 
  ...
  for (i in 1:dim(replicate.unique)[1]) {
    ...
    if (sum(ground.truth$hgvs_pro==replicate$aaChg[i])==1) {
...
save(all.premode, all.baseline, all.replicates, file = 'figs/05.CXCR4.replicates.RData')
npoints <- c('all', 0:max(all.replicates[[1]]$observations))
...
write.csv(to.plot, file = 'figs/05.CXCR4.replicates.csv')
library(ggplot2)
...
# ggsave("figs/05.CXCR4.enzy.replicates.pdf", height = 5, width = 6)
...
p3 <- protein.visualize('P60484', 'CXCR4', replicate.unique, 'PreMode.Experiment.RMSE.diff',
...
# ggsave(filename = 'figs/05.CXCR4.enzy.replicates.PreMode.pdf', p, width = 20, height = 5)
# p2 <- protein.visualize('P60484', 'CXCR4 : Experiment RMSE', replicate.unique, 'Experiment.RMSE',
#                         fill.colors = c("purple", "yellow", "black"),
#                         fill.values = c(0, 0.75, 1.5))
# p3 <- protein.visualize('P60484', 'CXCR4 : PreMode', replicate.unique, 'PreMode.RMSE',
#                         fill.colors = c("purple", "yellow", "black"),
#                         fill.values = c(0, 0.75, 1.5))
library(patchwork)
p <- (p1 + p2) / p3
ggsave(filename = 'figs/05.CXCR4.replicates.PreMode.pdf', p, width = 20, height = 10)
