...
write.csv(seq.df, '/share/vault/Users/gz2294/combining-evolutionary-and-assay-labelled-data/fasta/all.fasta', quote = F, row.names = F)
# process for each data
genes <- c('PTEN', 'NUDT15', 'CCR5', 'CXCR4', 'GCK', 'CYP2C9', 'ASPA', 'SNCA')
for (gene in genes) {
  for (fold in 0:4) {
    training <- read.csv(paste0('/share/vault/Users/gz2294/Data/DMS/MAVEDB/', gene, '/train.seed.', fold, '.csv'), 
                         row.names = 1)
    nscores <- sum(startsWith(colnames(training), 'score'))
    for (s in 1:nscores) {
      target.dir <- paste0('/share/vault/Users/gz2294/combining-evolutionary-and-assay-labelled-data/data/', gene, '.fold.', fold, '.score.', s)
