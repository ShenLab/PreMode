...
half_lr=$(printf "%.1e" "$(echo "scale=10; $(printf "%f" "$lr") / 2" | bc)")
...
do
  ...
  sed -i "s|num_workers: "$nworkers"|num_workers: 0|g" $1/$gene.yaml
  # change loaded msa
...
do
  ...
  sed -i "s|lr_warmup_steps: "$lr_warmup_steps"|lr_warmup_steps: 0|g" $1/$gene.yaml
...
do
  ...
  sed -i "s|PrimateAI.Random.ESM|MAVEDB/"$gene"|g" $1/$gene.yaml
...
do
  ...
  sed -i "s|PrimateAI.Random.ESM|Itan.CKB.Cancer.good.batch/pfams.0.8/"$gene"|g" $1/$gene.yaml
  # sed -i "s|BinaryClassification|Tanh|g" $1/$gene.yaml
  # get the size of training data
  data_size=$(cat /share/vault/Users/gz2294/Data/DMS/Itan.CKB.Cancer.good.batch/pfams.0.8/$gene/training.csv | wc -l)
  # batch size should roughly be 1/20 of the data size, but should not be larger than 8
  target_batch_size=$(($data_size*3/80))
  if [ $target_batch_size -gt 8 ]; then
    target_batch_size=8
  fi
  sed -i "s|batch_size: "$batch_size"|batch_size: "$target_batch_size"|g" $1/$gene.yaml
  # estimate the number of batches in one epoch
  target_num_batches=$(($data_size*3/4/$target_batch_size))
  # num saved batches should be 4 times lower
  sed -i "s|num_save_batches: "$num_save_batches"|num_save_batches: "$(($target_num_batches/4))"|g" $1/$gene.yaml
...
do
  ...
  sed -i "s|PrimateAI.Random.ESM|"$gene"|g" $1/$gene.yaml
