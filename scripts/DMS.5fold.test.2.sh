...
for fold in {0..4}
  do
    ...
    if [[ $gene == "Itan.CKB.Cancer" || $gene == "ICC" ]]; then
      for task in $(cat scripts/gene.pfams.txt); do
        echo "Begin "$task
	...
	logdir=$(cat $1/$task.5fold/$task.fold.$fold.yaml | grep log_dir | sed 's/.*: //')
        ...
        if [ -f $logdir/model.epoch.$num_epochs.pt ]; then
          python -W ignore::UserWarning:torch_geometric.data.collate:147 train.py --conf $1/$task.5fold/$task.fold.$fold.yaml --mode test
	fi
      done
    else
      echo "Begin "$gene
      ...
      num_epochs=$(cat $1/$task.5fold/$task.fold.$fold.yaml | grep num_epochs | sed 's/.*: //')
      if [ -f $logdir/model.epoch.$num_epochs.pt ]; then
        python -W ignore::UserWarning:torch_geometric.data.collate:147 train.py --conf $1/$gene.5fold/$gene.fold.$fold.yaml --mode test
      fi
    fi
  done
done
