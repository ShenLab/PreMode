...
cd /share/pascal/Users/gz2294/PreMode.final
...
for gene in PTEN PTEN.bin CCR5 CXCR4 NUDT15 VKORC1 DDX3X $(cat scripts/pfams.txt) AAV GB1 B_LAC fluorescence $(cat /share/pascal/Users/gz2294/ld1/Data/DMS/Hsu_NBT/useful.data.csv)
do
  # use original yaml as template
  cp $1/pretrain.seed.0.yaml $1/$gene.yaml
  # ngpu should be 1
  sed -i "s|ngpus: 4|ngpus: 1\nuse_lora: |g" $1/$gene.yaml
...
if [ "$loss_fn" == "combined_loss" ] || [ "$loss_fn" == "weighted_combined_loss" ]; then
  for gene in $(cat scripts/pfams.txt)
  do
...
for gene in PTEN PTEN.bin CCR5 CXCR4 NUDT15 VKORC1 DDX3X $(cat scripts/pfams.txt) $(cat /share/pascal/Users/gz2294/ld1/Data/DMS/Hsu_NBT/useful.data.human.csv)
do
  # change load model
  ...
  if [[ "PTEN CCR5 CXCR4 NUDT15 DDX3X" == *"$gene"* ]]; then
      sed -i "s|num_save_batches: "$num_save_batches"|num_save_batches: "$(($num_save_batches/5))"|g" $1/$gene.yaml
  else
      sed -i "s|num_save_batches: "$num_save_batches"|num_save_batches: "$(($num_save_batches/20))"|g" $1/$gene.yaml
  fi
done

for gene in $(cat /share/pascal/Users/gz2294/ld1/Data/DMS/Hsu_NBT/useful.data.other.csv)
do
  data_size=$(cat /home/gz2294/Data/DMS/$gene/train.seed.0.csv | wc -l)
  # warm up steps should be 20 times lower
...
for gene in PTEN PTEN.bin CCR5 CXCR4 NUDT15 VKORC1 DDX3X $(cat /share/pascal/Users/gz2294/ld1/Data/DMS/Hsu_NBT/useful.data.human.csv)
do
...
for gene in AAV GB1 B_LAC fluorescence $(cat /share/pascal/Users/gz2294/ld1/Data/DMS/Hsu_NBT/useful.data.other.csv)
do
...
# change seed
for gene in PTEN PTEN.bin CCR5 CXCR4 NUDT15 VKORC1 DDX3X $(cat scripts/pfams.txt) AAV GB1 B_LAC fluorescence
do
  # use original yaml as template
...
for gene in $(cat /share/pascal/Users/gz2294/ld1/Data/DMS/Hsu_NBT/useful.data.csv)
do
  mkdir $1/$gene.5fold/
